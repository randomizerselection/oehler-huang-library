(function attachFinancialDecisionCourseMap(global) {
  const termBank = {
    ...
    investmentPlan: ["investment plan", "投资计划", "An investment plan records goals, horizon, risk limits, product rules and review arrangements before products are chosen."],
    constraint: ["investment constraint", "投资限制", "An investment constraint is a limit such as a short horizon, liquidity need, loss limit or restricted product type."],
    ...
    productComparison: ["product comparison", "产品比较", "Product comparison evaluates alternatives using the same goal, risk, cost, liquidity and evidence criteria."],
    ...
    familyStrategy: ["family investment strategy", "家庭投资策略", "A family investment strategy connects several goals to suitable products, risk limits, contributions and review rules."],
  ...
  const units = [
    { unit: 1, semester: 1, lessons: [1, 8], title: "Personal Investment Foundations", summary: "Students connect family goals, time, inflation, compounding and risk to a written investment plan before comparing products.", unitOutput: "A goals-and-time-horizon investment plan for a mock family." },
    { unit: 2, semester: 1, lessons: [9, 17], title: "Investment Choices for Families", summary: "Students compare cash, bonds, shares and funds, then combine product knowledge with fees, diversification and suitable investing methods.", unitOutput: "A product-fit comparison memo for three mock family goals." },
  ...
  const lessonSpecs = [
    ...
    { anchor: "Family investment policy", role: "synthesis case", q: "Why should an investor write a plan before choosing products?", zh: "为什么应先制定投资计划再选择产品？", hook: "A popular fund appears before a family has agreed its goals or risk limits. What should happen first?", core: "A written investment plan turns goals and constraints into decision rules before products or market excitement influence the choice.", terms: ["investmentPlan", "constraint", "decisionRule"], formula: "No new formula; write rules for goal, horizon, liquidity, risk, cost and review.", task: "Complete a one-page investment plan for a mock family using evidence from Lessons 1-7.", naive: "A plan is unnecessary if the investment has performed well recently.", need: "Students need agreed goals, constraints, acceptable risks, product boundaries and review arrangements.", output: "Write three investment-plan rules and justify the most important one.", avoid: "Do not select named products; finish Unit 1 with rules that later product lessons will use." },
    ...
    { anchor: "Three family profiles", role: "synthesis case", q: "Which investment mix fits different family goals?", zh: "哪种投资组合适合不同的家庭目标？", hook: "Three families have the same amount but different goals. Should they hold the same investments?", core: "A suitable product mix connects each goal to horizon, liquidity, loss capacity, diversification and cost.", terms: ["assetAllocation", "investorProfile", "portfolio"], formula: "Asset-class weight = asset-class value / total portfolio value x 100.", task: "Build and defend different high-level product mixes for short-, medium- and long-horizon profiles.", naive: "The portfolio with the highest expected return is best for all three families.", need: "Students need each goal's horizon, flexibility, liquidity need, risk capacity and product evidence.", output: "Write a product-fit comparison memo for the three profiles.", avoid: "Use broad asset classes and mock profiles; do not recommend named products or personalised portfolios." },
    ...
    { anchor: "Fund-share-bond comparison", role: "product comparison", q: "How can funds, shares and bonds be compared fairly?", zh: "如何公平比较基金、股票和债券？", hook: "A bond has a clear yield, a fund has broad holdings and a share has strong growth. Which comparison is fair?", core: "Fair product comparison uses the same goal and evaluates return source, downside, liquidity, cost, diversification and evidence limitations.", terms: ["productComparison", "opportunityCost", "suitability"], formula: "Use relevant earlier return, yield, cost and weight formulas only with aligned data.", task: "Complete a comparison matrix for one mock profile using frozen product evidence.", naive: "The product with the highest recent return is the best alternative.", need: "Students need the same goal, date, currency, horizon, risk categories, costs and product terms.", output: "Choose compare further, consider, watch or avoid and justify the product trade-off.", avoid: "Do not force unlike return measures into one misleading ranking." },
  ...
  function sourceTypes(spec) {
    ...
    if (/fund|financial product|product comparison/.test(spec.role)) {
    ...
    return ["mock family or investor profile", "teacher-frozen product, market or economic evidence", "official or regulator investor-education source"];
  }
  ...
  const courseMap = {
    ...
    practicalInvestingBoundary: {
      positiveMethod: "Use goals, horizon, liquidity, risk capacity, product evidence, company evidence, price, cost and limitations to choose a defensible next action.",
    ...
    investmentWorkflow: [
      { step: 1, title: "Start with the goal", studentAction: "State what the money is for, when it is needed and how flexible the goal is.", evidenceCheck: "Goal, amount, horizon and liquidity need are explicit.", decisionOutput: "A defined investment problem rather than a product preference." },
      ...
      { step: 3, title: "Understand the product", studentAction: "Identify what is owned, how return may arise, costs, liquidity and major risks.", evidenceCheck: "Official product terms and dated evidence are recorded.", decisionOutput: "A product evidence summary." },
    ...
    simpleLessonStructure: [
      { label: "Hook", purpose: "Begin with a family, product, market, company or portfolio dilemma students can answer before definitions.", studentQuestion: "What would I decide first?" },
    ...
    decisionFirstSyllabus: {
      ...
      lessonContract: [
        "Starter dilemma: a goal, product, market, company or portfolio question students can answer before definitions.",
