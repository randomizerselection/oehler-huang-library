...
Authenticated selectors also offer **Top Homework**. It opens a full-screen classroom praise view for the selected class. The latest non-QQ scored assignment is ranked by `score / score_max`; it shows the first five students plus everyone tied with the fifth student, so a large shared top score is never cut off. The latest QQ assignment is a separate structured-question completion group where submitted/late status—not the score—is the reward criterion. Missing and unscored students are not displayed in the quiz group. Current servers supply both groups in the protected selector-session payload; a compatibility path reads protected teacher homework profiles when a page was refreshed before its older local server process restarted.

The Top Homework screen loops `assets/Elmer-Bernstein.mp3` while it is visible. Closing the screen, changing class, resetting or leaving the selector stops it immediately. The soundtrack follows the selector's existing Sound toggle.
