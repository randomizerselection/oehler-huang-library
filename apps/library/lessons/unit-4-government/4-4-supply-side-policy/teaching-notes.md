...
The same diagram controls are used in lessons 1, 3 and 4. Later policy decks also use full-slide questions, shorter teaching chunks and immediate MCQ, yes/no and written checks. Lesson 4's education question is the main writing task; its inflation question is optional consolidation.

## Source and design basis

Content uses the local 2027–2029 syllabus, **4.4.1–4.4.3**, and the definitions archive entries for productivity, the PPC, factor immobility and subsidies. The four-mark retraining task is teacher-written; its explanation is adapted from **2023ON-22 Q3(c)**. The continuation retains that original six-mark question. Lesson 4 uses **2024ON-21 Q3(d)** for education evaluation.

The teaching pattern follows the recent money lessons: full-slide questions, explicit definitions, local concept photographs and formative checks immediately after teaching. Diagram presentation follows the A-level multiplier's large graph, adjacent step text, reference curve and replay controls. Existing supply-side photos and their catalogue credits are retained.
