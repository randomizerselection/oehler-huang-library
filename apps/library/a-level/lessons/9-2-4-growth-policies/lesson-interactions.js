...
(function () {
  ...
  function visualLayout(s){
    if(s.visual==='policy-hook')return `<p class="illustration-label">Illustrative economies · real-output index · productive capacity = 100</p><div class="policy-puzzle">${s.economies.map((p,i)=>`<article><h2>${e(p.name)}</h2><div class="policy-meter"><div style="width:${p.output}%">${p.output}</div><span>100</span></div><p>${e(p.condition)}</p><p class="policy-answer" data-reveal-step="${i+1}">${e(p.answer)}</p></article>`).join('')}</div><p class="hook-verdict" data-reveal-step="3">Choose the policy that removes the constraint: <strong>insufficient demand</strong> or <strong>limited productive capacity</strong>.</p>${revealControls()}`;
    if(s.visual==='cause-chains')return `<div class="cause-paths">${s.items.slice(1).map((p,i)=>`<article data-reveal-step="${i+1}"><h2>${e(p.heading)} <span lang="zh-Hans">${e(p.headingZh)}</span></h2><div class="cause-path">${p.nodes.map((n,j)=>`<p>${e(n)}</p>${j<3?`<span class="cause-link">${e(p.links[j])}<b>→</b></span>`:''}`).join('')}</div></article>`).join('')}</div>${revealControls()}`;
    if(s.visual==='essay-map')return `${questionStrip(s)}<div class="essay-route">${s.items.slice(1).map((p,i)=>`<article data-reveal-step="${i+1}"><span>${String(i+1).padStart(2,'0')}</span><h2>${e(p.heading)}</h2><p>${e(p.text)}</p></article>`).join('')}</div><p class="essay-allocation">At least two policies · AO1 + AO2: 14 · AO3: 6</p>${revealControls()}`;
  ...
  function showModel(slide){
    ...
    const modelSlides=lesson.slides.filter(s=>s.modelParagraph&&s.modelGroup===group).sort((a,b)=>a.modelOrder-b.modelOrder);
    const paragraphs=[];
    ...
    const scene=lesson.slides.find(s=>s.id==='capacity-growth-diagram').scene;
    const figure=document.createElement('figure');figure.className='model-diagram';
    figure.innerHTML=window.EconDiagrams.markup(scene,'complete-policy-model')+'<figcaption>Skills and useful infrastructure shift AS₀ to AS₁, increasing potential output from Y<sub>fe,0</sub> to Y<sub>fe,1</sub>. With AD₀ unchanged, equilibrium moves from E₀ to E₁: real output rises to Y₁ and the price level falls to P₁, while some new capacity remains unused. This distinguishes potential growth from its full use.</figcaption>';
    modelDialog.querySelector('.modal-content section').after(figure);
    window.EconDiagrams.update(figure,scene,2,true);
...
// Readable Chinese support for the three learning objectives.
for(const [i,item] of window.ALEVEL_LESSON.slides.find(s=>s.id==="learning-objectives").items.entries()){const line=document.createElement("span");line.className="objective-zh";line.lang="zh-Hans";line.textContent=item.textZh;document.querySelectorAll("#learning-objectives .objective-list article p")[i].append(line);}
