...
const workshop=pathToFileURL(path.resolve(__dirname,'../a-level/lessons/9-1-3-full-employment-essay/index.html')).href;
...
test('income gaps: authentic questions, reversible modelling and source controls',async({page})=>{
  ...
  expect(data.slides).toHaveLength(21);
  expect(data.slides.at(-2).id).toBe('closing-deflationary-gap');
  expect(data.slides.some(s=>s.layout==='essay')).toBe(false);
  ...
  const essayData=await page.evaluate(()=>window.ALEVEL_LESSON);
  expect(essayData.slides).toHaveLength(15);
  ...
  for(const id of ['paper-deflationary-distance','paper-inflationary-gap','exit-assessment']){
    await page.goto(id==='exit-assessment'?workshop:lesson);
    await show(id);
  ...
  for(const [url,content] of [[lesson,data],[workshop,essayData]]){
  await page.goto(url);
  for(const s of content.slides){
    const i=content.slides.indexOf(s),max=s.scene?s.scene.steps.length-1:s.reveal?(s.kind==='table'?s.table.length-1:s.items.length):['steps','chain'].includes(s.kind)?s.items.length-1:0;
    await show(s.id,max);
...
test('income gaps: new layouts on narrow screens and reduced motion',async({page})=>{
  ...
  for(const [id,step] of [['paper-four-model-diagnosis',3],['paper-inflationary-gap',3],['gaps-compared',2],['gaps-both',3],['inflationary-gap-diagram',2],['output-gaps',2],['full-employment-unemployment',2],['uk-deflationary-example',0],['canada-inflationary-example',0],['lesson-conclusion',0]]){
    await page.goto((id.startsWith('paper-four')||id==='lesson-conclusion'?workshop:lesson)+`#${id}/${step}`);
    expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1)).toBe(true);
  }
  await page.goto(workshop+'#paper-four-model-diagnosis/3');
