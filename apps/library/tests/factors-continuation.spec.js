...
test('typed source dialogs preserve question context, keyboard state and focus',async({page})=>{
  ...
  expect(await page.evaluate(()=>location.hash)).toBe('#17');
...
test('factor continuation: reversible predictions, correct counts, study views and classroom fit', async ({page}) => {
  ...
  const shape=await page.evaluate(()=>({slides:IGCSE.lesson.slides.length,exit:IGCSE.lesson.slides[22].title}));
  expect(shape).toEqual({slides:24,exit:'Identify two reasons why people become entrepreneurs. [2]'});
  for(const [slide,last] of [[6,3],[8,2],[11,4],[14,3],[15,3]]){
    ...
    if(slide===6||slide===15)await expect(film.locator('[data-move]')).toHaveCount(12);
    if(slide===15){
