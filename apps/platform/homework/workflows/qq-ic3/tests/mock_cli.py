"""Mock transport CLI for fixture tests. Reads a scenario JSON (env
...
in the internal dialect (the same dialect qq_cli.py speaks). Never talks to QQ.
"""
import json
import os
import sys
...
def get_self(config):
    ...
    if mode == 'transport-down':
        sys.stderr.write('qq-cli: cannot reach OneBot endpoint http://127.0.0.1:3000 '
                         '(connection refused or unreachable)\n')
        print(json.dumps({'success': False, 'error': 'cannot reach OneBot endpoint'}))
        sys.exit(1)
    user = config.get('userId', 'TEACHER_QQ')
    if mode == 'wrong-account':
        user = 'OTHER_QQ_999'
    if mode == 'snake':
        print(json.dumps({'success': True, 'user_id': user}))
    elif mode == 'flat':
        print(json.dumps({'success': True, 'userId': user}))
    elif mode == 'nested':
        # Production shape: identifiers nested inside the result object.
        print(json.dumps({'result': [{'profile': {
            'userId': user, 'nickname': 'Samuel'}}]}))
    else:
        print(json.dumps({'success': True, 'result': [{'userId': user, 'nickname': 'Samuel'}]}))
...
def main():
    ...
    if tuple(args[:2]) == ('drive', 'upload'):
        ...
        if drive.get('mode') == 'no-ids':
            ...
            return
        print(json.dumps({'success': True, 'result': {
            'fileId': drive.get('fileId', 'FILE1'),
            'spaceId': drive.get('spaceId', 'SPACE1'),
            'fileName': drive.get('fileName', 'question.png')}}))
    elif tuple(args[:2]) == ('drive', 'info'):
        info = config.get('driveInfo', {})
        if info.get('mode') == 'fail':
            fail('drive info failed')
        print(json.dumps({'success': True, 'result': {
            'dentryId': info.get('dentryId', 'DENTRY1'),
            'spaceId': info.get('spaceId', 'SPACE1'),
            'fileId': info.get('fileId', 'FILE1'),
            'name': info.get('name', 'question.png')}}))
        ...
        path = args[args.index('--file-path') + 1]
        ocr = config.get('ocrText', {})
        if isinstance(ocr, dict) and ocr.get('error'):
            fail(ocr['error'])
        lines = ocr.get(path) if isinstance(ocr, dict) else None
        if isinstance(ocr, dict) and ocr.get('default'):
            lines = ocr['default']
        print(json.dumps({'success': True, 'result': {'file': path, 'lineCount': len(lines or []),
