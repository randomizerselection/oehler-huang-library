...
class PreflightTests(unittest.TestCase):
    def test_preflight_success(self):
        ...
        self.assertEqual(report['checks']['account'], {'userId': 'TEACHER_QQ'})
        self.assertEqual(report['checks']['platformSchemaVersion'], 19)
    ...
    def test_preflight_wrong_account_fails_closed_without_state_writes(self):
        fixture = build_workspace(self, {'getSelf': {'mode': 'wrong-account'}})
    ...
    def test_preflight_transport_down_exit6_distinct_from_sign_in(self):
        fixture = build_workspace(self, {'getSelf': {'mode': 'transport-down'}})
        result = run_script(fixture, 'run_guard.py', 'preflight')
        self.assertEqual(result.returncode, 6, result.stdout)
        self.assertIn('transport down', result.stdout)
        self.assertNotIn('sign-in required', result.stdout)
...
class RunLockTests(unittest.TestCase):
    def test_begin_overlap_end_lifecycle(self):
        ...
        owner_env = {'QQ_RUN_OWNER_PID': str(os.getpid())}
...
class ConfigHookTests(unittest.TestCase):
    def test_qq_config_path_env_overrides_config(self):
        fixture = build_workspace(self)
        alternate = fixture.tmp / 'alternate.json'
        config = json.loads((fixture.ws / 'config.json').read_text(encoding='utf-8'))
        config['onebotToken'] = 'alternate-token'
        alternate.write_text(json.dumps(config), encoding='utf-8')
        os.environ['QQ_CONFIG_PATH'] = str(alternate)
        try:
            module = fresh_import(fixture, 'fetch')
            self.assertEqual(module.CONFIG['onebotToken'], 'alternate-token')
        finally:
            del os.environ['QQ_CONFIG_PATH']
        module = fresh_import(fixture, 'fetch')
        self.assertEqual(module.CONFIG['onebotToken'], 'fixture-token')
