...
from fixtures import (CLASS_IC31, EMMA, HEADER, LEO, PLATFORM_TITLE, ASSIGNED_ON, WENDY,
...
def emma_message(mid='m1', sender='QQ_EMMA', time='2026-09-16 10:00:00'):
...
class DecisionValidationTests(WorkspaceCase):
    def test_out_of_scope_student_is_rejected(self):
        fixture = build_workspace(self)
        incoming = message('m1', WENDY['qqId'], candidates=[{'key': WENDY['key']}],
        ...
        students = {WENDY['key']: {'key': WENDY['key'], 'class': 'S3.6',
                                   'qqId': WENDY['qqId']}}
        with self.assertRaises(ValueError) as caught:
            update.verified_student({'studentKey': WENDY['key']}, incoming, students)
        self.assertIn("outside ['IC3.1']", str(caught.exception))

    def test_unresolved_identity_is_rejected(self):
        fixture = build_workspace(self)
        write_pending(fixture, [emma_message(sender='QQ_UNKNOWN')])
        ...
        write_pending(fixture, [message('m1', 'QQ_UNKNOWN', candidates=[])])
        result = self.prepare(fixture)
        self.assertNotEqual(result.returncode, 0)
    ...
    def test_old_status_mismatch_aborts_commit_and_rolls_back(self):
        ...
        incoming = message('m1', 'QQ_NEW', candidates=[{'key': LEO['key']}],
        ...
        links = connection.execute(
            "SELECT * FROM student_integrations WHERE external_id='QQ_NEW'").fetchall()
        connection.close()
...
class MultiAssignmentMessageTests(unittest.TestCase):
    def test_one_message_can_back_two_assignments(self):
        fixture = build_workspace(self, {})
        write_pending(fixture, [message('m-both', 'QQ_EMMA', candidates=[
            {'key': EMMA['key'], 'accountId': EMMA['accountId'], 'classId': CLASS_IC31,
             'class': 'IC3.1', 'name': '张梦', 'english': 'Emma', 'qqId': 'QQ_EMMA'}])])
