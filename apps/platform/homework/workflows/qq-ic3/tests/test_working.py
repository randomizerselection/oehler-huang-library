...
def seed_awaiting_working(fixture, received_at, teacher_accepted=False):
    ...
    entry = {'action': 'needs_work', 'studentKey': EMMA['key'], 'assignment': HEADER,
             'senderId': EMMA['qqId'], 'responseFormat': 'letter_only',
...
class WorkingTimingTests(unittest.TestCase):
    ...
    def test_working_arriving_between_fetch_and_send_suppresses(self):
        new_message = wire_message('m-working', 'QQ_EMMA', now_local(),
    ...
    def test_list_only_answer_gets_a_full_sentence_rewrite_request(self):
        fixture = build_workspace(self, {'pages': empty_pages()})
        seed_awaiting_working(fixture, now_local(dt.timedelta(minutes=-3)))
        ledger = load_state(fixture, 'ledger.json')
        entry = ledger['processed']['m1']
        entry['responseFormat'] = 'list_only'
        entry['workingCheckEvidence'] = 'counter-arguments written as a list of headings'
        ...
        self.assertEqual(len(sends), 1)
        self.assertEqual(sends[0]['content'],
                         'Hi Emma, I received your points for Homework 1 — Inflationary gap (Q19). Please write them in full sentences and send a photo so I can record a complete submission.')
        # One request only, then quiet.
        self.assertEqual(json.loads(run_working(fixture).stdout)['sent'], 0)
        self.assertEqual(len(read_log(fixture, 'sends.log')), 1)
