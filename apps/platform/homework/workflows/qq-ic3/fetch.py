"""Read QQ private messages (via qq-cli/OneBot) and the platform roster; never send messages."""
...
def cli(args):
    ...
    timeout = 40 if args[:3] == ['chat', 'message', 'ocr-text'] else 240
    proc = subprocess.run([CONFIG['cli'], *args], capture_output=True, encoding='utf-8', timeout=timeout)
    try:
        result = json.loads(proc.stdout)
    except ValueError:
        raise RuntimeError('QQ transport returned non-JSON output: ' + proc.stderr[:500])
...
def main():
    ...
    for page in range(50):
        ...
        if not isinstance(result.get('hasMore'), bool) or not isinstance(result.get('conversationMessagesList'), list):
            raise RuntimeError('Unexpected QQ pagination response; cannot establish completeness')
    ...
    for conversation in conversations:
        ...
        for msg in conversation.get('messages', []):
            ...
            if not candidates and not re.search(r'作业|homework|assignment|答案|question|图片消息|文件消息|\[File:', content, re.I):
                continue
    ...
    batch = {'batchId': uuid.uuid4().hex[:12], 'runId': os.environ.get('QQ_RUN_ID') or None, 'start': start, 'end': end, 'platformSchemaVersion': schema_version(), 'messages': pending}
