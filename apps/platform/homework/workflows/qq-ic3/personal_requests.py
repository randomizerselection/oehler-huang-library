"""Read-only IC3 personal-request attention report.

The raw archive state/personal-replies.json covers all private QQ chats. This
reporter only surfaces unanswered personal messages from platform-linked IC3
students in state/personal-attention.json. It never sends, answers, or files
anything.
"""
import argparse
...
def excerpt_of(message):
    ...
    if text.startswith('[图片消息]'):
        text = '[Image attachment — see the original QQ chat]'
    elif ' fileId:' in text:
...
def current_items():
    ...
    linked = {s['qqId'] for s in students if s.get('qqId')}
    automated = reply_queue.automated_ids()
    self_id = CONFIG['selfQqId']
    items = {}
