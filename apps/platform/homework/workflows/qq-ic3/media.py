"""Download image and file evidence from the pending student submission batch."""
import hashlib
import json
import re
...
for message in batch['messages']:
    # QQ students often send photos as file segments; both kinds are evidence.
    resources = [{'id': r['resourceId'], 'type': r.get('resourceType')}
                 for r in message['resources']
                 if r.get('resourceType') in ('image', 'file') and r.get('resourceId')]
    if not resources:
        resources = [{'id': found, 'type': 'image'}
                     for found in re.findall(r'mediaId=([^\)]+)', message['text'])]
    for index, resource in enumerate(resources):
        path = ROOT / 'state' / 'media' / (hashlib.sha256(message['messageId'].encode()).hexdigest()[:16] + f'-{index}.jpg')
        path.parent.mkdir(parents=True, exist_ok=True)
        if not path.exists():
            response = cli(['chat', 'message', 'download-media', '--type', 'mediaId', '--resource-id', resource['id'], '--resource-type', resource['type'], '--message-id', message['messageId'], '--open-conversation-id', message['conversationId'], '--output', str(path)])
