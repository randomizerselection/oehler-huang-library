...
from fetch import ROOT, CONFIG, TZ, cli, save
...
def incoming(state):
    return {key(cid,mid): {'conversationId':cid,'title':c['title'],'messageId':mid,**m}
        ...
        if m.get('senderOpenDingTalkId')!=CONFIG['selfQqId']}

def capture_fresh(snapshot,end):
    lock=ROOT/'state/personal-replies.lock'
    handle=os.open(lock,os.O_CREAT|os.O_EXCL|os.O_WRONLY)
    try:return capture(snapshot,end)
    finally:os.close(handle);lock.unlink()

def automated_ids():
    ids=set()
    for filename in ('receipts.json','working-followups.json','reminder-plan.json'):
...
def apply_reviews(state, decisions):
    messages=incoming(state); automated=automated_ids(); changes=[]
    ...
    for d in decisions:
        ...
        status=d['status']
        if status not in ('open','routine','answered'):raise ValueError('Invalid review status')
        ...
        if status=='answered':
            ...
            if (reply.get('senderOpenDingTalkId')!=CONFIG['selfQqId'] or
...
def render(state):
    ...
    for k,m,d in active:
        ...
        if text.startswith('[图片消息]'):text='[Image attachment — see the original QQ chat]'
...
<style>body{font:17px/1.5 system-ui;margin:40px auto;max-width:900px;padding:0 24px;background:#f3f6fa;color:#172c45}h1{margin-bottom:0}h2{margin:8px 0}article{background:white;border:1px solid #d7e0ea;border-radius:12px;padding:24px;margin:18px 0}.status{color:#9b3c00;font-weight:650}time{color:#596879;font-size:14px}blockquote{white-space:pre-wrap;border-left:3px solid #abc1d8;margin:15px 0;padding-left:16px}header p{color:#596879}</style><header><h1>Needs my reply</h1>'''
    ...
    page+='<p>Reply in the original QQ conversation. Items stay here until a personal reply addresses them. Automated receipts do not clear requests. This local page updates after each scheduled check; reopen or refresh it to see changes.</p></header>'
    ...
    try:
        if args.command=='prepare':result=prepare()
            ...
            if args.command=='apply':
                result['changes']=apply_reviews(state,json.loads(Path(args.decisions).read_text(encoding='utf-8')))
                save(STATE,state)
            if args.command in ('sync','apply'):result['native']=sync_native(state)
            render(state)
