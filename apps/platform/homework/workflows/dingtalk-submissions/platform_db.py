...
def connect(readonly=False):
    ...
    connection = sqlite3.connect(target, uri=readonly, timeout=15, factory=ManagedConnection)
...
def backup(label='dingtalk-write'):
    ...
    with connect(readonly=True) as source, sqlite3.connect(destination, factory=ManagedConnection) as target:
        source.backup(target)
    return destination


def roster(classes=PLATFORM_CLASSES, connection=None):
    placeholders = ','.join('?' for _ in classes)
    owned = connection is None
    db = connection or connect(readonly=True)
    try:
        require_schema(db)
        rows = db.execute(f'''
...
def student_by_key(key, connection=None):
    matches = [student for student in roster(connection=connection) if student['key'] == key]
    if len(matches) != 1:
...
def require_in_scope_student(account_id, class_id=None, connection=None):
    """Revalidate an active S3.3/S3.4 membership inside the caller's transaction."""
    owned = connection is None
    connection = connection or connect(readonly=True)
    try:
        ...
        if len(rows) != 1:
            raise ValueError('Student is not uniquely active in the authorized S3.3/S3.4 scope')
        return dict(rows[0])
    finally:
        if owned:
            connection.close()
...
def require_recipient(student_key, sender_id, connection=None):
    """Bind an outbound destination to the current verified platform identity."""
    if not sender_id:
        raise ValueError('Outbound action lacks a DingTalk recipient')
    owned = connection is None
    connection = connection or connect(readonly=True)
    try:
        student = student_by_key(student_key, connection=connection)
        scoped = require_in_scope_student(student['accountId'], student['classId'], connection=connection)
        if not scoped.get('dingtalk_id') or scoped['dingtalk_id'] != sender_id:
            raise ValueError('Outbound recipient no longer matches the verified platform identity')
        return student
    finally:
        if owned:
            connection.close()
...
def classify_dingtalk_senders(sender_ids, connection=None):
    """Return verified active class memberships for stable DingTalk sender IDs."""
    ids = sorted({value for value in sender_ids if value})
    if not ids:
        return {}
    ...
    try:
        rows = connection.execute(f'''
            SELECT si.external_id,c.name AS class_name
            FROM student_integrations si
            JOIN accounts a ON a.id=si.account_id
            JOIN class_memberships m ON m.account_id=a.id
            JOIN classes c ON c.id=m.class_id
            WHERE si.provider='dingtalk' AND si.external_id IN ({placeholders})
              AND a.role='student' AND a.status='active' AND m.status='active'
        ''', tuple(ids)).fetchall()
        result = {}
        for row in rows:
            result.setdefault(row['external_id'], set()).add(row['class_name'])
        return {sender_id: sorted(classes) for sender_id, classes in result.items()}
    finally:
        if owned:
            connection.close()
...
def current_counts():
    with connect(readonly=True) as db:
        require_schema(db)
        rows = db.execute('''SELECT h.status,COUNT(*) AS count FROM homework_submissions h
            JOIN classes c ON c.id=h.class_id WHERE c.name IN ('S3.3','S3.4') GROUP BY h.status''').fetchall()
