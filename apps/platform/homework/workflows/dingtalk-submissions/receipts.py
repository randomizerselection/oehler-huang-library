...
from platform_db import mark_confirmation, require_recipient, verify_homework
...
def submitted_records():
    ...
    for mid,item in ledger['processed'].items():
        ...
        if item.get('action')=='submitted':
            ...
            result[key]={**item,'sourceMessageId':item.get('messageId') or mid.split('#',1)[0]}
    return result
    ...
    for key,item in records.items():
        ...
        if not task:
            ...
            if entry.get('status') == 'sending':
                ...
                entry['lastError']='Process stopped after send intent was saved but before a task ID was persisted'
                save(STATE,state)
            ...
            verify_saved(item)
            entry={**entry,'status':'sending','attemptedAt':now.isoformat(),'sourceMessageId':item['sourceMessageId'],'senderId':item['senderId'],'idempotencyKey':'submission-'+key,'text':text}
        ...
        if message_id and conversation_id:
            ...
            remote_status=str(field(status,'status') or field(status,'sendStatus') or '').lower()
