...
def multi_page_scenario():
    page0 = {'cursor': '0', 'hasMore': True, 'nextCursor': 'c1', 'conversations': [
        conversation('conv-emma', [
            wire_message('m1', 'DING_EMMA', '2026-09-18 08:00:00'),
            wire_message('m2', 'DING_EMMA', '2026-09-18 08:00:00'),  # timestamp tie
        ]),
        conversation('conv-leo', [wire_message('m3', 'DING_LEO', '2026-09-18 08:01:00')],
                     title='李雷'),
    ]}
    page1 = {'cursor': 'c1', 'hasMore': False, 'conversations': [
        conversation('conv-emma', [
            wire_message('m2', 'DING_EMMA', '2026-09-18 08:00:00'),  # duplicate across pages
            wire_message('m4', 'DING_EMMA', '2026-09-18 08:02:00'),
        ]),
        conversation('conv-group', [wire_message('m5', 'DING_EMMA', '2026-09-18 08:03:00')],
...
class FetchTests(unittest.TestCase):
    def test_multi_page_dedup_and_timestamp_ties(self):
        ...
        result = run_script(fixture, 'fetch.py', extra_env={'S36_RUN_ID': 'run-test-1'})
...
class FetchContactSearchToleranceTests(unittest.TestCase):
    def test_failing_contact_search_leaves_message_unresolved_not_aborted(self):
        page = {'cursor': '0', 'hasMore': False, 'conversations': [
            conversation('conv-emoji', [wire_message('m-emoji', 'DING_UNKNOWN',
                                                     '2026-09-18 09:00:00', content='[图片消息] photo',
                                                     sender='jellyfish nickname')],
                         title='jellyfish nickname'),
            conversation('conv-emma', [wire_message('m-emma', 'DING_EMMA',
