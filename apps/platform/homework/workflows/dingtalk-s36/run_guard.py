"""Run-level lock, preflight checks, and run records for the S3.6 automation.

Commands:
  preflight  Environment, state writability, platform schema, and signed-in
             account checks. Exit 0 ok; 4 sign-in required; 5 account mismatch.
...
def self_identity():
    """Read-only account check through the configured shim. Never retries and
    never runs any other dws command."""
    ...
    try:
        ...
        raise RuntimeError('DingTalk returned non-JSON output for get-self: ' + output[:300])
    ...
    def visit(node, depth):
        if depth > 6 or ('corp' in found and 'user' in found):
            return
        if isinstance(node, dict):
            corp = node.get('corpId') or node.get('corp_id')
            user = node.get('userId') or node.get('user_id')
            if corp:
                found.setdefault('corp', corp)
            if user:
            ...
            for value in node:
                visit(value, depth + 1)
    visit(payload, 0)
    if found.get('corp') or found.get('user'):
        return found.get('corp'), found.get('user')
    raise RuntimeError('get-self response lacks corpId/userId: ' + json.dumps(payload, ensure_ascii=True)[:300])
...
def preflight():
    ...
    corp, user = self_identity()
    checks['account'] = {'corpId': corp, 'userId': user}
    if corp != CONFIG.get('expectedCorpId') or user != CONFIG.get('expectedUserId'):
        emit({'status': 'error', 'error': 'account mismatch', 'checks': checks,
              'expected': {'corpId': CONFIG.get('expectedCorpId'), 'userId': CONFIG.get('expectedUserId')}})
        return 5
...
def begin():
    ...
    # agent registers its own long-lived PID via S36_RUN_OWNER_PID; without it
    # the lock owner is this (immediately exiting) process.
    owner_pid = int(os.environ.get('S36_RUN_OWNER_PID') or os.getpid())
