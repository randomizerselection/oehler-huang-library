...
const scratch=path.join(repo,'tmp/income-expenditure-video');
const outDir=path.join(repo,'authoring/a-level/outputs/videos');
...
if(process.argv.includes('--preview')){
  for(const [i,s] of timeline.entries()){
    await page.evaluate(t=>renderFrame(t),s.brand?s.start+2:s.end-.1);
...
const output=path.join(outDir,'Income-expenditure-multiplier.mp4');
const stagingOutput=path.join(scratch,'Income-expenditure-multiplier-render.mp4');
const fps=24,frames=Math.ceil(cursor*fps);
const encoder=spawn(ffmpeg,['-y','-hide_banner','-loglevel','warning','-f','image2pipe','-framerate',String(fps),'-vcodec','mjpeg','-i','pipe:0','-i',path.join(scratch,'narration.wav'),'-c:v','libx264','-preset','veryfast','-crf','19','-pix_fmt','yuv420p','-threads','4','-c:a','aac','-b:a','128k','-movflags','+faststart','-shortest','-metadata','title=The multiplier: the income-expenditure approach','-metadata','comment=Male neural narration: Microsoft en-GB-RyanNeural. Original three-note intro and outro. Fixed-price closed-economy teaching model.',stagingOutput],{stdio:['pipe','ignore','pipe']});
...
try{
  ...
  for(let f=0;f<frames;f++){
    ...
    const moving=item.brand?(relative<.8||time>item.end-.8):(lessonScene===11||([3,4,6,8,9,10].includes(lessonScene)&&relative<5.6));
